...
class MaterialsTextProcessor:
    ...
    def process(self, tokens, exclude_punct=False, convert_num=True, normalize_materials=True, remove_accents=True,
        ...
        for i, tok in enumerate(tokens):
            ...
            elif (len(tok) == 1 or (len(tok) > 1 and tok[0].isupper() and tok[1:].islower()))                     and tok not in self.ELEMENTS and tok not in self.SPLIT_UNITS                     and self.ELEMENT_DIRECTION_IN_PAR.match(tok) is None:
